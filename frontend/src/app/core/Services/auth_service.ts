...
import { ApiResponse, UserProfile } from './api_response_models';
...
export class AuthService {
    ...
    private readonly storageKey = 'tournament_frontend_session';
    ...
    login(payload: LoginParameters): Observable<ApiResponse<AuthData>> {
        return this.http
            .post<ApiResponse<AuthData>>(`${environment.apiUrl}/auth/login`, payload)
            .pipe(tap((response) => this.setSession(response.data)));
    }

    register(payload: RegisterParameters): Observable<ApiResponse<{ user: UserProfile }>> {
